    ...
    <div className="w-full h-full flex flex-col md:py-10 md:gap-36">
      {articles.map((article) => (
        ...
        >
          ...
          <div
            className="w-full h-full flex items-center -mt-6 bg-primary text-lg text-justify text-white rounded-3xl [box-shadow:1px_0px_15px_8px_rgba(0,0,0,0.21)] md:translate-y-5 md:w-[100%] md:-mt-0   md:rounded-[4rem]"
            aria-describedby={`article-text-${article.id}`}
          >
            <p
              id={`article-text-${article.id}`}
              className="px-5 py-10 md:px-12"
            >
              {article.text}
            </p>
          </div>
