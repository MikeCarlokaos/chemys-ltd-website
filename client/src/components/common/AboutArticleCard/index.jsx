    ...
    <div className="flex flex-col md:gap-36">
      {articles.map((article) => (
        ...
        >
          ...
          <div
            className="w-full h-full flex items-center p-5 -mt-10 bg-white text-lg text-justify text-black  rounded-3xl [box-shadow:1px_0px_15px_8px_rgba(0,0,0,0.21)] md:mt-0 md:p-10 md:bg-transparent md:text-white md:rounded-none md:shadow-none"
            aria-describedby={`article-text-${article.id}`}
          >
            <p id={`article-text-${article.id}`}>{article.text}</p>
          </div>
        </div>
      ))}
    </div>
